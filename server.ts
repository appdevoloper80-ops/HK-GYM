...
// AI Coach Chat API Endpoint with ultra-fast smart routing and high-availability fallback
app.post("/api/coach", async (req, res) => {
  ...
  if (query.includes("contact") || query.includes("phone") || query.includes("whatsapp") || query.includes("call") || query.includes("number") || query.includes("mobile") || query.includes("support")) {
    return res.json({
...
async function setupVite() {
  // Statically serve the root /assets folder so files like /gym-media/gym_video.mp4 can be requested
  app.use("/gym-media", express.static(path.join(process.cwd(), "assets")));
